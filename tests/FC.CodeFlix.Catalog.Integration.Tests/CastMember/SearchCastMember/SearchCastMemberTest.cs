...
public class SearchCastMemberTest : IDisposable
{
        ...
        )
    {
        ...
        var castMemberNameList = new List<string>()
        {
            "Action",
            "Horror",
            "Horror - Robots",
        ...
        var examples = _fixture.GetCategoryModelList(castMemberNameList);
    ...
    public async Task SearchCastMemberWhenReceivesValidSearchInput_ReturnOrderedList(string orderBy, string direction)
    {
        ...
        var input = new SearchCastMemberInput(page: 1,
            perPage: examples.Count,
            orderBy: orderBy,
            order: direction == "asc" ? SearchOrder.ASC : SearchOrder.DESC);

        var expectedList = _fixture.CloneCastMemberListOrdered(examples, orderBy, input.Order);
