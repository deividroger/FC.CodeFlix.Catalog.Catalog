...
{
    public GenreModelOutput(Guid id, string name, bool isActive, DateTime createdAt, IEnumerable<GenreModelOutputCategory> categories)
    {
        Id = id;
        Name = name;
        IsActive = isActive;
    ...
    public static GenreModelOutput FromGenre(DomainEntity.Genre genre)
        => new(genre.Id,
            ...
            genre.IsActive,
            genre.CreatedAt,
            genre.Categories.Select(category => new GenreModelOutputCategory(category.Id, category.Name)));
